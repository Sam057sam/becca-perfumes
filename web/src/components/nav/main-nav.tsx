...
type Brand = { name?: string | null; logo?: string | null };

export function MainNav({ brand }: { brand?: Brand }) {
                ...
                <SheetTitle>
                  {brand?.logo ? (
                    <span className="inline-flex items-center">
                      <Image src={brand.logo} alt={brand?.name || "Company"} width={120} height={28} className="h-7 w-auto" />
                    </span>
                  ) : (
                    brand?.name ?? "Company"
                  )}
